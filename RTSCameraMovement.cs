...
public class RTSCameraMovement : MonoBehaviour 
{
	...
	float minZoom = 50.0f; //For Clipping 
	...
	float minimumAngularIncrement = 1.0f;
	float rotationRange = 30.0f; // in degrees not rads
	...
	bool moving = false;
	bool snap = true;
	float snapRotationVelocity = 2.0f;
	...
	void Start() 
	{
	 	...
	 	viewPosition = camera.WorldToViewportPoint(target.position);
